...
/usr/bin/find $base_path/uploads/buddypress/members -type f -regex '.*\.\(avi\|mov\|mp4\|jpg\|jpeg\|gif\|png\|webp\)$' -mmin +$minutes -print | head -500 | while read -r i
do
    echo "----------------------------------------------------"
    echo "Check file => $i"
    remote_path="${i/#$base_path}"
    final_remote_path="${remote_path#/}"
    user_folder="/uploads/buddypress/members/"
