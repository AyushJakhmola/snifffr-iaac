resource "aws_codestarconnections_connection" "connect" {
  name          = format("%s-%s-connection", local.environment, local.name)
  provider_type = "GitHub"
}

resource "aws_codepipeline" "codepipeline" {
  name     = format("%s-%s-pipeline", local.environment, local.name)
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ProjectName = format("%s-%s-codebuild", local.environment, local.name)
      }
    }
  }
...
resource "aws_iam_role_policy" "codepipeline_policy" {
  name   = format("%s-%s-pipeline-policy", local.environment, local.name)
