...
source "amazon-ebs" "basic-ami" {
  ami_name             = "snifffr-backapp-packer-{{timestamp}}"
...
# adding script bp-core-avatars.php
  ...
  provisioner "shell" {
    inline = ["sudo mv /tmp/root /var/spool/cron/crontabs/root"]
  }

  provisioner "shell" {
    inline = ["sudo chown -R root:crontab /var/spool/cron/crontabs/root"]
  }
